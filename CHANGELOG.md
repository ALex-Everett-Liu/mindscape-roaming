...
- **FTS5 search** (`core-fts-search`): simple queries now supplement FTS5 prefix results with a `LIKE %query%` fallback for substring matches that FTS5 tokenization would miss (e.g., searching "usage" now finds "ccusage"); FTS5 ranked results appear first, LIKE results fill remaining slots; boolean expressions skip this supplement
- **FTS5 search**: row mapping now includes `node_size` and `category` fields (previously omitted, masked by `as` cast)
...
- **Block Timestamps plugin** (`core-block-timestamps`, core, disabled by default): Right-click "Show Block Info" opens a popup showing the block's creation time, last-update time, and ID
...
- **Context menu**: Added `max-height` + `overflow-y: auto` so long menus scroll instead of overflowing the viewport; fixed estimated-height positioning to use measured DOM dimensions
...
- **Image Gallery plugin** (`third-party-image-gallery`, community, disabled by default): Browse all images under a node (itself + full subtree of descendants) with left/right arrow key navigation and fullscreen zoom
  ...
  - **Image gathering**: Parse `![](path)` syntax from the focused node and all its descendants (subtree walk); deduplicate by path
